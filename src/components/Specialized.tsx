"use client";
import { courses, quarter45 } from "@/contents";
import {
  Box,
  Button,
  Flex,
...
const SpecializedTracks = () => (
  <Box>
    ...
    <Text mt={"2"}>
      After completing the first three quarters the participants will select one
      or more specializations consisting of two courses each:
    </Text>
    <Flex wrap={["wrap", "wrap", "nowrap"]}>
      <Box mr={["2", "4", "8"]}>
        <Box boxShadow="lg" borderWidth={1} my={"6"} p={["4", "8"]}>
          <Text
            fontSize={"md"}
            fontWeight={"semibold"}
            mt={"5"}
      ...
      <List spacing={3}>{courses.map(TrackCard)}</List>
    </Flex>
  </Box>
);
